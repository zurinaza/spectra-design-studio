const phases = [
  ...
  { key: 'P', name: 'Prepare flow', sub: 'Build and validate the BFD/PFD', color: 'pink' },
  { key: 'E', name: 'Enter data', sub: 'Define streams and evidence', color: 'cyan' },
  { key: 'C', name: 'Calculate', sub: 'Check DOF, balance and size', color: 'yellow' },
...
const evidenceCategories = [
  { id: 'brief', label: 'Feed and product requirements', source: 'Project brief', priority: 'Core' },
  { id: 'properties', label: 'Physical and thermodynamic properties', source: 'Validated database', priority: 'As needed' },
  { id: 'methods', label: 'Design methods and correlations', source: 'Textbook / standard', priority: 'Core' },
  { id: 'equilibrium', label: 'Equilibrium and kinetic data', source: 'Journal / experiment', priority: 'When applicable' },
  { id: 'limits', label: 'Equipment operating limits', source: 'Vendor / industry', priority: 'Recommended' },
  { id: 'costs', label: 'Cost and utility data', source: 'Vendor / current industry', priority: 'Later stage' },
  { id: 'assumptions', label: 'Missing information and assumptions', source: 'Declared assumption', priority: 'Optional' }
];
...
function createDefaults() {
  return { spectraVersion: 2, phase: 0, operation: 'evaporation', equipment: 'falling_film', mode: 'Single effect', configuration: 'Vacuum', selectionRationale: '', projectTitle: 'Concentrate a heat-sensitive aqueous feed', feedDescription: 'Aqueous solution containing a non-volatile solute', throughput: 1000, flowUnit: 'kg/h', targetComponent: 'Non-volatile solute', productTarget: 'Increase solute from 10 to 40 wt%', constraints: 'Use saturated steam; minimise thermal exposure', basis: '1 hour of operation', assumptions: ['steady', 'nonvolatile'], components: 'Non-volatile solute, Water', compositionBasis: 'Mass fraction', processBlocks: [{ id: 'b1', kind: 'unit', operation: 'evaporation', equipment: 'falling_film', mode: 'Single effect', configuration: 'Vacuum', tag: 'EV-101', target: true }], streamData: {}, processValidation: null, diagramView: 'BFD', networkBalance: null, dataEntries: [], showDataQuality: false, F: 1000, z: 0.10, x: 0.40, y: 0, cp: 4.0, dTfeed: 55, latent: 2257, sizing: {}, sensitivity: 110, simValues: {}, simulation: null, economics: { values: { ...economicDefaults }, result: null }, checks: { units: false, balance: false, range: false, safety: false, operability: false }, decision: '', reflection: '', diagram: baseDiagram() };
}
...
let state = { ...defaults, ...stored, checks: { ...defaults.checks, ...(stored.checks || {}) }, sizing: stored.sizing || {}, simValues: stored.simValues || {}, economics: { values: { ...economicDefaults, ...(stored.economics?.values || {}) }, result: stored.economics?.result || null }, diagram: { ...baseDiagram(), ...(stored.diagram || {}), positions: { ...(stored.diagram?.positions || {}) } }, assumptions: stored.assumptions || [...defaults.assumptions], dataEntries: stored.dataEntries || [], processBlocks: stored.processBlocks || defaults.processBlocks, streamData: stored.streamData || {} };
if (!stored.processBlocks) { const legacyOperation = operations[state.operation] ? state.operation : 'evaporation'; state.processBlocks = [{ id: 'b1', kind: 'unit', operation: legacyOperation, equipment: state.equipment, mode: state.mode, configuration: state.configuration, tag: state.diagram.unitTag || `${operations[legacyOperation].icon}-101`, target: true }]; }
if (!stored.spectraVersion) { const oldMap = [0, 2, 3, 3, 4, 6]; state.phase = oldMap[Math.min(5, Number(stored.phase) || 0)]; }
state.spectraVersion = 2;
...
function targetBlock() { return state.processBlocks.find(block => block.kind === 'unit' && block.target) || state.processBlocks.find(block => block.kind === 'unit'); }
function normalizeProcess() {
  if (!Array.isArray(state.processBlocks) || !state.processBlocks.length) state.processBlocks = [{ id: 'b1', kind: 'unit', operation: state.operation || 'evaporation', equipment: state.equipment || 'falling_film', mode: state.mode || 'Single effect', configuration: state.configuration || 'Vacuum', tag: state.diagram?.unitTag || 'EV-101', target: true }];
  state.processBlocks.forEach((block, index) => {
    block.id ||= uid();
    block.kind ||= 'unit';
    if (block.kind === 'unit') {
      block.operation = operations[block.operation] ? block.operation : 'evaporation';
      const op = operations[block.operation];
      block.equipment = op.equipment.some(item => item.id === block.equipment) ? block.equipment : op.equipment[0].id;
      block.mode = op.modes.includes(block.mode) ? block.mode : op.modes[0];
      block.configuration = op.configurations.includes(block.configuration) ? block.configuration : op.configurations[0];
      block.tag ||= `${op.icon}-${101 + index}`;
    } else block.tag ||= block.kind === 'pump' ? `P-${101 + index}` : `E-${101 + index}`;
  });
  const units = state.processBlocks.filter(block => block.kind === 'unit');
  if (units.length && !units.some(block => block.target)) units[0].target = true;
  if (units.filter(block => block.target).length > 1) units.slice(1).forEach(block => block.target = false);
  const target = targetBlock();
  if (target) { state.operation = target.operation; state.equipment = target.equipment; state.mode = target.mode; state.configuration = target.configuration; state.diagram.unitTag = target.tag; }
  state.streamData ||= {}; state.diagramView ||= 'BFD'; state.components ||= 'Non-volatile solute, Water';
}
function componentsList() { return String(state.components || '').split(',').map(item => item.trim()).filter(Boolean); }
function visibleBlocks(view = state.diagramView) { return view === 'BFD' ? state.processBlocks.filter(block => block.kind === 'unit') : state.processBlocks; }
function processStreams(view = 'PFD') {
  const blocks = visibleBlocks(view), streams = [];
  if (!blocks.length) return streams;
  streams.push({ key: `source-${blocks[0].id}`, from: 'Feed boundary', to: blocks[0].tag, role: 'feed', defaultLabel: 'Raw feed' });
  blocks.slice(0, -1).forEach((block, index) => streams.push({ key: `${block.id}-${blocks[index + 1].id}`, from: block.tag, to: blocks[index + 1].tag, role: 'main', defaultLabel: `${block.tag} outlet to ${blocks[index + 1].tag}` }));
  streams.push({ key: `${blocks.at(-1).id}-product`, from: blocks.at(-1).tag, to: 'Product boundary', role: 'product', defaultLabel: 'Main product' });
  blocks.filter(block => block.kind === 'unit').forEach(block => streams.push({ key: `${block.id}-side`, from: block.tag, to: 'Side-product boundary', role: 'side', unitId: block.id, defaultLabel: `${block.tag} separated stream` }));
  return streams.map((stream, index) => ({ ...stream, number: `S-${String(index + 1).padStart(2, '0')}`, data: state.streamData[stream.key] || {} }));
}
function ensureStreamData() { processStreams('PFD').forEach(stream => { const isNew = !state.streamData[stream.key]; state.streamData[stream.key] ||= {}; state.streamData[stream.key].label ||= stream.defaultLabel; if (isNew && stream.role === 'feed') { state.streamData[stream.key].flow = Number(state.throughput) || ''; state.streamData[stream.key].composition = `${state.z ?? 0.1}, ${1 - (state.z ?? 0.1)}`; } if (isNew && stream.role === 'product') state.streamData[stream.key].composition = `${state.x ?? 0.4}, ${1 - (state.x ?? 0.4)}`; if (isNew && stream.role === 'side') state.streamData[stream.key].composition = `${state.y ?? 0}, ${1 - (state.y ?? 0)}`; }); }
function invalidateProcess() { state.processValidation = null; state.networkBalance = null; state.sizing = {}; state.simulation = null; }
function processFingerprint() { ensureStreamData(); return JSON.stringify({ blocks: state.processBlocks, labels: processStreams('PFD').map(stream => [stream.key, stream.data.label]) }); }
function processIssues() {
  ensureStreamData(); const issues = [], units = state.processBlocks.filter(block => block.kind === 'unit'), tags = state.processBlocks.map(block => block.tag.trim().toUpperCase());
  if (!units.length) issues.push('Add at least one separation unit.');
  if (state.processBlocks.some(block => !block.tag.trim())) issues.push('Give every equipment item a tag.');
  if (new Set(tags).size !== tags.length) issues.push('Equipment tags must be unique.');
  if (!units.some(block => block.target)) issues.push('Select the unit to be sized and simulated.');
  const labels = processStreams('PFD').map(stream => stream.data.label?.trim());
  if (labels.some(label => !label)) issues.push('Label every material stream.');
  if (new Set(labels.map(label => label?.toLowerCase())).size !== labels.length) issues.push('Use a distinct label for every material stream.');
  if ((state.selectionRationale || '').trim().length < 20) issues.push('Justify the selected process arrangement and target equipment.');
  return issues;
}
function processValidated() { return state.processValidation?.status === 'pass' && state.processValidation.fingerprint === processFingerprint(); }
function parseComposition(value) {
  if (value === undefined || value === null || String(value).trim() === '') return null;
  const values = String(value).split(',').map(item => Number(item.includes('=') ? item.split('=').at(-1).trim() : item.trim()));
  if (values.length !== Math.max(2, componentsList().length) || values.some(item => !Number.isFinite(item) || item < 0)) return null;
  const sum = values.reduce((a, b) => a + b, 0), normalized = sum > 1.5 ? values.map(item => item / 100) : values;
  return Math.abs(normalized.reduce((a, b) => a + b, 0) - 1) <= 0.01 ? normalized : null;
}
function dofSummary() {
  const streams = processStreams('PFD'), n = Math.max(2, componentsList().length), unknownFlow = streams.filter(stream => !hasNumber(stream.data.flow) || stream.data.flowCalculated).length, unknownComp = streams.filter(stream => !parseComposition(stream.data.composition) || stream.data.compositionCalculated).length * (n - 1), equations = state.processBlocks.length * n;
  return { unknowns: unknownFlow + unknownComp, equations, dof: unknownFlow + unknownComp - equations };
}
function hasNumber(value) { return value !== '' && value !== null && value !== undefined && Number.isFinite(Number(value)); }
function networkFingerprint() { return JSON.stringify({ process: processFingerprint(), streams: processStreams('PFD').map(stream => [stream.key, stream.data.flow, stream.data.composition]) }); }
function streamByKey(key) { return processStreams('PFD').find(stream => stream.key === key); }
function solveNetworkBalances() {
  ensureStreamData(); const results = [], errors = [];
  for (let pass = 0; pass < 2; pass += 1) state.processBlocks.forEach((block, index) => {
    if (block.kind === 'unit') return;
    const incomingKey = index ? `${state.processBlocks[index - 1].id}-${block.id}` : `source-${block.id}`, outgoingKey = index < state.processBlocks.length - 1 ? `${block.id}-${state.processBlocks[index + 1].id}` : `${block.id}-product`, incoming = state.streamData[incomingKey] || {}, outgoing = state.streamData[outgoingKey] || {};
    if (parseComposition(incoming.composition) && !parseComposition(outgoing.composition)) { outgoing.composition = incoming.composition; outgoing.compositionCalculated = true; }
    if (parseComposition(outgoing.composition) && !parseComposition(incoming.composition)) { incoming.composition = outgoing.composition; incoming.compositionCalculated = true; }
  });
  state.processBlocks.forEach((block, index) => {
    const incomingKey = index ? `${state.processBlocks[index - 1].id}-${block.id}` : `source-${block.id}`;
    const outgoingKey = index < state.processBlocks.length - 1 ? `${block.id}-${state.processBlocks[index + 1].id}` : `${block.id}-product`;
    const incoming = state.streamData[incomingKey] || {}, outgoing = state.streamData[outgoingKey] || {};
    if (!hasNumber(incoming.flow) || !parseComposition(incoming.composition)) { errors.push(`${block.tag}: define its inlet flow and composition.`); return; }
    if (block.kind !== 'unit') {
      outgoing.flow = Number(incoming.flow); outgoing.flowCalculated = true;
      if (!outgoing.composition || outgoing.compositionCalculated) { outgoing.composition = incoming.composition; outgoing.compositionCalculated = true; }
      results.push({ tag: block.tag, kind: block.kind, inlet: Number(incoming.flow), main: Number(outgoing.flow), side: 0, closure: 0 }); return;
    }
    const side = state.streamData[`${block.id}-side`] || {}, zin = parseComposition(incoming.composition), xout = parseComposition(outgoing.composition), yout = parseComposition(side.composition);
    if (!xout || !yout) { errors.push(`${block.tag}: enter valid compositions for its main and separated outlet streams.`); return; }
    const F = Number(incoming.flow), z = zin[0], x = xout[0], y = yout[0], denominator = x - y;
    if (Math.abs(denominator) < 1e-9) { errors.push(`${block.tag}: outlet compositions must be different.`); return; }
    const P = F * (z - y) / denominator, R = F - P;
    if (![P, R].every(value => Number.isFinite(value) && value >= 0)) { errors.push(`${block.tag}: specified compositions give a negative or impossible stream flow.`); return; }
    outgoing.flow = Number(P.toFixed(6)); outgoing.flowCalculated = true; side.flow = Number(R.toFixed(6)); side.flowCalculated = true;
    results.push({ tag: block.tag, kind: block.kind, inlet: F, main: P, side: R, closure: Math.abs((P + R - F) / Math.max(F, 1e-9) * 100) });
  });
  if (errors.length) return { status: 'fail', errors, results, fingerprint: networkFingerprint() };
  const target = targetBlock(), targetResult = results.find(item => item.tag === target?.tag);
  if (target && targetResult) {
    const index = state.processBlocks.findIndex(block => block.id === target.id), inKey = index ? `${state.processBlocks[index - 1].id}-${target.id}` : `source-${target.id}`, outKey = index < state.processBlocks.length - 1 ? `${target.id}-${state.processBlocks[index + 1].id}` : `${target.id}-product`;
    state.F = Number(state.streamData[inKey].flow); state.z = parseComposition(state.streamData[inKey].composition)[0]; state.x = parseComposition(state.streamData[outKey].composition)[0]; state.y = parseComposition(state.streamData[`${target.id}-side`].composition)[0];
  }
  return { status: 'pass', errors: [], results, fingerprint: networkFingerprint(), runAt: new Date().toISOString() };
}
...
function dataIssues(entries = state.dataEntries) {
  ...
  entries.forEach((entry, index) => {
    ...
    if (!String(entry.value || '').trim()) issues.push(`${label}: extract or paste the relevant value, equation or method.`);
    if (entry.inputMethod === 'url' && !safeHttpUrl(entry.url)) issues.push(`${label}: provide a valid http/https source link.`);
    if (entry.inputMethod === 'file' && !entry.attachmentName) issues.push(`${label}: attach a file or change the evidence method.`);
...
function readFileAsDataURL(file) { return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(reader.result); reader.onerror = () => reject(new Error('The selected file could not be read.')); reader.readAsDataURL(file); }); }
...
function phaseBlockers(index) {
  ...
  if (index === 1) {
    blockers.push(...processIssues().slice(0, 4));
    if (!processValidated()) blockers.push('Validate the process flow after its latest change.');
  }
  if (index === 2) {
    ...
    const streams = processStreams('PFD'), feed = streams.find(stream => stream.role === 'feed'), dof = dofSummary();
    if (componentsList().length !== 2) blockers.push('List exactly two components for this transparent binary-balance model.');
    if (!feed || !hasNumber(feed.data.flow) || Number(feed.data.flow) <= 0 || !parseComposition(feed.data.composition)) blockers.push('Enter a positive feed flow and a valid feed composition.');
    if (streams.some(stream => stream.data.composition && !parseComposition(stream.data.composition))) blockers.push('Each entered composition must sum to 1.00 or 100%.');
    if (dof.dof !== 0) blockers.push(`Resolve the degree of freedom: DOF is ${dof.dof}.`);
    blockers.push(...dataIssues().slice(0, 2));
  }
  if (index === 3) {
    if (!state.networkBalance || state.networkBalance.fingerprint !== networkFingerprint() || state.networkBalance.status !== 'pass') blockers.push('Solve and close the process-stream balances.');
...
function phaseUnlocked() { return true; }
...
function gatePanel() {
  ...
  if (!blockers.length) return `<div class="gate-panel ready" id="stageGate"><div class="gate-icon">✓</div><div><strong>Stage ready</strong><span>The evidence required for this stage is complete.</span></div></div>`;
  return `<div class="gate-panel revise" id="stageGate"><div class="gate-icon">!</div><div><strong>Information still needed</strong><span>You may continue to preview the next stage. These items remain flagged in your project.</span><ul>${blockers.map(item => `<li>${esc(item)}</li>`).join('')}</ul></div></div>`;
}
function refreshGate() {
  const gate = document.getElementById('stageGate');
  if (gate) gate.outerHTML = gatePanel();
  const next = document.getElementById('nextBtn');
  if (next) { next.disabled = false; next.textContent = state.phase === phases.length - 1 ? 'Open portfolio' : phaseReady(state.phase) ? 'Save & continue →' : 'Continue with flags →'; }
}
function shell(title, description, body) {
  const phase = phases[state.phase], ready = phaseReady(state.phase); document.body.dataset.phase = phase.color;
  return `<div class="phase-title-row"><div><div class="phase-kicker">${phase.key} · ${phase.name.toUpperCase()}</div><h1>${title}</h1><p>${description}</p></div><span class="step-chip">STAGE ${state.phase + 1} / ${phases.length}</span></div>${spectraFlow()}<div class="phase-callout"><div class="big-letter">${phase.key}</div><div><strong>${phase.name}</strong><span>${phase.sub}. Every saved decision becomes part of the design portfolio.</span></div></div>${body}${gatePanel()}<div class="button-row"><button class="ghost" id="prevBtn" ${state.phase === 0 ? 'disabled' : ''}>← Previous stage</button><button class="primary" id="nextBtn">${state.phase === phases.length - 1 ? 'Open portfolio' : ready ? 'Save & continue →' : 'Continue with flags →'}</button></div>`;
}
...
function legacyRenderDataRegister() {
  const rows = state.dataEntries.map((entry, index) => { const method = entry.inputMethod || 'text', methodLabel = method === 'url' ? 'URL / DOI' : method === 'file' ? 'Attachment' : 'Typed / pasted'; return `<tr><td><span class="data-title">${esc(entry.name)}</span><span class="evidence-method-badge ${method}">${methodLabel}</span><span class="data-meta">${esc(entry.conditions)}</span></td><td>${esc(entry.value || 'Not extracted yet')} ${esc(entry.unit || '')}</td><td><strong>${esc(entry.sourceType)}</strong><span class="data-meta">${esc(entry.reference || 'Reference incomplete')}</span>${method === 'url' && safeHttpUrl(entry.url) ? `<button class="mini-btn evidence-open" data-source-url="${esc(safeHttpUrl(entry.url))}">Open link ↗</button>` : ''}${method === 'file' && entry.attachmentName ? `<button class="mini-btn evidence-open" data-attachment="${index}">Open ${esc(entry.attachmentName)}</button>` : ''}</td><td><span class="quality-pill ${(entry.status || 'Provisional').toLowerCase()}">${esc(entry.status || 'Provisional')}</span></td><td><div class="table-actions"><button class="mini-btn" data-edit="${index}">Edit</button><button class="mini-btn danger" data-delete="${index}">Delete</button></div></td></tr>`; }).join('');
...
function renderEvidenceSpaces() {
  return `<div class="evidence-space-intro"><div><strong>Evidence collection spaces</strong><span>Add information only where it is relevant to your design. Empty conditional or optional spaces do not prevent you from continuing.</span></div><div class="evidence-intro-actions"><span class="advisory-chip">Advisory, not locked</span><button class="ghost" id="continueEvidence">Continue to calculations with flags →</button></div></div><div class="evidence-spaces">${evidenceCategories.map(category => { const entries = state.dataEntries.map((entry, index) => ({ entry, index })).filter(item => item.entry.category === category.id), complete = entries.length > 0; return `<section class="evidence-space ${complete ? 'has-evidence' : ''}"><div class="evidence-space-head"><div><span>${esc(category.priority)}</span><h3>${esc(category.label)}</h3></div><b>${complete ? '✓ Added' : '○ Empty'}</b></div><p>Suggested source: <strong>${esc(category.source)}</strong></p><div class="evidence-space-items">${entries.length ? entries.map(({ entry, index }) => `<button data-edit="${index}"><span>${esc(entry.name || 'Untitled evidence')}</span><small>${esc(entry.inputMethod === 'url' ? 'URL / DOI' : entry.inputMethod === 'file' ? 'Attachment' : 'Typed / pasted')} · ${esc(entry.status || 'Provisional')}</small></button>`).join('') : '<span>No information supplied. You may leave this blank if it is not applicable yet.</span>'}</div><button class="mini-btn evidence-add" data-add-category="${category.id}">+ Add text, link or file</button></section>`; }).join('')}</div>`;
}

function renderDataRegister() {
  const rows = state.dataEntries.map((entry, index) => { const method = entry.inputMethod || 'text', methodLabel = method === 'url' ? 'URL / DOI' : method === 'file' ? 'Attachment' : 'Typed / pasted', category = evidenceCategories.find(item => item.id === entry.category)?.label || 'Other evidence'; return `<tr><td><span class="data-title">${esc(entry.name)}</span><span class="evidence-method-badge ${method}">${methodLabel}</span><span class="data-meta">${esc(category)} · ${esc(entry.conditions)}</span></td><td>${esc(entry.value || 'Not extracted yet')} ${esc(entry.unit || '')}</td><td><strong>${esc(entry.sourceType)}</strong><span class="data-meta">${esc(entry.reference || 'Reference incomplete')}</span>${method === 'url' && safeHttpUrl(entry.url) ? `<button class="mini-btn evidence-open" data-source-url="${esc(safeHttpUrl(entry.url))}">Open link ↗</button>` : ''}${method === 'file' && entry.attachmentName ? `<button class="mini-btn evidence-open" data-attachment="${index}">Open ${esc(entry.attachmentName)}</button>` : ''}</td><td><span class="quality-pill ${(entry.status || 'Provisional').toLowerCase()}">${esc(entry.status || 'Provisional')}</span></td><td><div class="table-actions"><button class="mini-btn" data-edit="${index}">Edit</button><button class="mini-btn danger" data-delete="${index}">Delete</button></div></td></tr>`; }).join('');
  const issues = dataIssues();
  return `${renderEvidenceSpaces()}<div class="data-toolbar"><div class="quality-summary"><span>Total evidence records</span><strong>${state.dataEntries.length}</strong><span class="quality-pill ${issues.length ? 'provisional' : 'verified'}">${issues.length ? `${issues.length} flag${issues.length === 1 ? '' : 's'}` : 'Ready'}</span></div><button class="primary small" id="addDataBtn">+ Add other evidence</button></div>${state.dataEntries.length ? `<div class="data-table-wrap"><table class="data-table"><thead><tr><th>DATA / CATEGORY</th><th>INFORMATION</th><th>SOURCE / REFERENCE</th><th>STATUS</th><th>ACTIONS</th></tr></thead><tbody>${rows}</tbody></table></div>` : `<div class="data-empty"><strong>No evidence entered yet</strong><span>You may add evidence above or continue to preview the next stage. Missing information will remain flagged.</span></div>`}<button class="ghost data-check" id="dataCheckBtn">Review evidence flags</button>${state.showDataQuality ? `<div class="source-alerts">${issues.length ? issues.map(issue => `<div class="source-alert">${esc(issue)}</div>`).join('') : '<div class="source-alert good">✓ Every recorded item has traceable evidence, conditions and applicability.</div>'}</div>` : ''}`;
}

function legacyRenderEstablish() {
  const operation = operations[state.operation];
...
function legacyRenderCalculate() {
...
function optionList(items, selected) { return items.map(item => `<option value="${esc(typeof item === 'string' ? item : item.id)}" ${selected === (typeof item === 'string' ? item : item.id) ? 'selected' : ''}>${esc(typeof item === 'string' ? item : item.label)}</option>`).join(''); }
function renderBlockEditor(block, index) {
  const unit = block.kind === 'unit', op = unit ? operations[block.operation] : null, title = unit ? op.name : block.kind === 'pump' ? 'Pump' : 'Heat exchanger';
  return `<article class="process-block-card ${block.kind} ${block.target ? 'target' : ''}"><div class="block-card-head"><div><span>${unit ? 'SEPARATION UNIT' : 'ACCESSORY'}</span><strong>${esc(block.tag)} · ${title}</strong></div><div class="block-actions"><button class="mini-btn" data-move-block="${block.id}" data-direction="-1" ${index === 0 ? 'disabled' : ''} aria-label="Move left">←</button><button class="mini-btn" data-move-block="${block.id}" data-direction="1" ${index === state.processBlocks.length - 1 ? 'disabled' : ''} aria-label="Move right">→</button><button class="mini-btn danger" data-remove-block="${block.id}" ${state.processBlocks.length === 1 ? 'disabled' : ''}>Remove</button></div></div><div class="field-grid compact-fields"><div class="field"><label>Equipment tag</label><input data-block-field="tag" data-block-id="${block.id}" value="${esc(block.tag)}"></div>${unit ? `<div class="field"><label>Unit operation</label><select data-block-field="operation" data-block-id="${block.id}">${optionList(Object.entries(operations).map(([id, item]) => ({ id, label: item.name })), block.operation)}</select></div><div class="field"><label>Equipment type</label><select data-block-field="equipment" data-block-id="${block.id}">${optionList(op.equipment, block.equipment)}</select></div><div class="field"><label>Operating mode</label><select data-block-field="mode" data-block-id="${block.id}">${optionList(op.modes, block.mode)}</select></div><div class="field"><label>Configuration</label><select data-block-field="configuration" data-block-id="${block.id}">${optionList(op.configurations, block.configuration)}</select></div><label class="target-choice"><input type="radio" name="targetUnit" data-target-block="${block.id}" ${block.target ? 'checked' : ''}><span>Size and simulate this unit</span></label>` : `<div class="accessory-note"><strong>${block.kind === 'pump' ? 'Pressure-changing accessory' : 'Heating/cooling accessory'}</strong><span>Assumed to pass total mass and composition unchanged for the material balance.</span></div>`}</div></article>`;
}
function bfdStream(blocks, index, role, unitId = '') {
  const pfd = processStreams('PFD');
  if (role === 'feed') return pfd.find(stream => stream.role === 'feed');
  if (role === 'product') return pfd.find(stream => stream.role === 'product');
  if (role === 'side') return pfd.find(stream => stream.key === `${unitId}-side`);
  return pfd.find(stream => stream.to === blocks[index + 1]?.tag) || { number: '—', data: { label: `Process stream to ${blocks[index + 1]?.tag}` } };
}
function renderProcessDiagram(view = state.diagramView, id = 'processSvg') {
  ensureStreamData(); const blocks = visibleBlocks(view), width = 1000, y = 145;
  if (!blocks.length) return '<div class="data-empty"><strong>Add a separation unit to begin.</strong></div>';
  const x = index => blocks.length === 1 ? 500 : 145 + index * (710 / (blocks.length - 1));
  const pfdStreams = processStreams('PFD'), streamFor = (index, role, unitId = '') => view === 'PFD' ? (role === 'feed' ? pfdStreams.find(stream => stream.role === 'feed') : role === 'product' ? pfdStreams.find(stream => stream.role === 'product') : role === 'side' ? pfdStreams.find(stream => stream.key === `${unitId}-side`) : pfdStreams.find(stream => stream.key === `${blocks[index].id}-${blocks[index + 1].id}`)) : bfdStream(blocks, index, role, unitId);
  const label = (stream, tx, ty, anchor = 'middle') => `<text x="${tx}" y="${ty}" text-anchor="${anchor}" class="edge-label"><tspan x="${tx}" dy="0">${esc(stream?.number || '—')}</tspan><tspan x="${tx}" dy="15">${esc((stream?.data?.label || stream?.defaultLabel || 'Unlabelled').slice(0, 34))}</tspan></text>`;
  let edges = '', feed = streamFor(0, 'feed'), product = streamFor(blocks.length - 1, 'product');
  edges += `<line x1="25" y1="${y}" x2="${x(0) - 62}" y2="${y}" class="diagram-edge" marker-end="url(#arrowhead)"/>${label(feed, 32, y - 25, 'start')}`;
  blocks.slice(0, -1).forEach((block, index) => { const stream = streamFor(index, 'main'); edges += `<line x1="${x(index) + 62}" y1="${y}" x2="${x(index + 1) - 62}" y2="${y}" class="diagram-edge" marker-end="url(#arrowhead)"/>${label(stream, (x(index) + x(index + 1)) / 2, y - 25)}`; });
  edges += `<line x1="${x(blocks.length - 1) + 62}" y1="${y}" x2="975" y2="${y}" class="diagram-edge" marker-end="url(#arrowhead)"/>${label(product, 968, y - 25, 'end')}`;
  blocks.forEach((block, index) => { if (block.kind === 'unit') { const stream = streamFor(index, 'side', block.id); edges += `<line x1="${x(index)}" y1="${y + 48}" x2="${x(index)}" y2="272" class="diagram-edge side-edge" marker-end="url(#arrowhead)"/>${label(stream, x(index), 300)}`; } });
  const nodes = blocks.map((block, index) => { const px = x(index), op = block.kind === 'unit' ? operations[block.operation] : null, detail = block.kind === 'unit' ? op.equipment.find(item => item.id === block.equipment)?.label : block.kind === 'pump' ? 'Pump' : 'Heat exchanger'; let shape = `<rect x="-60" y="-43" width="120" height="86" rx="10" class="node-shape ${block.kind}"/>`; if (block.kind === 'pump') shape = '<circle cx="0" cy="0" r="42" class="node-shape pump"/><path d="M -15 -15 L 22 0 L -15 15 Z" class="node-symbol"/>'; if (block.kind === 'heat_exchanger') shape = '<rect x="-48" y="-42" width="96" height="84" rx="8" class="node-shape heater"/><path d="M -31 -25 L 31 25 M 31 -25 L -31 25" class="node-symbol stroke"/>'; return `<g class="diagram-node" transform="translate(${px} ${y})">${shape}<text text-anchor="middle" class="node-label"><tspan x="0" dy="-4">${esc(block.tag)}</tspan><tspan x="0" dy="16">${esc(String(detail).slice(0, 21))}</tspan></text>${block.target ? '<circle cx="49" cy="-35" r="10" class="target-dot"/><text x="49" y="-31" text-anchor="middle" class="target-star">★</text>' : ''}</g>`; }).join('');
  return `<svg class="process-svg process-train-svg" id="${id}" viewBox="0 0 ${width} 340" role="img" aria-label="${view === 'PFD' ? 'Process flow diagram' : 'Block flow diagram'}"><defs><marker id="arrowhead" markerWidth="9" markerHeight="7" refX="8" refY="3.5" orient="auto"><polygon points="0 0, 9 3.5, 0 7"/></marker></defs><text x="25" y="28" class="diagram-title">${view} · USER-DEFINED PROCESS ORDER · ★ TARGET UNIT</text>${edges}${nodes}</svg>`;
}
function renderStreamLabelTable() {
  ensureStreamData(); return `<div class="data-table-wrap"><table class="data-table stream-label-table"><thead><tr><th>STREAM</th><th>FROM → TO</th><th>REQUIRED LABEL</th></tr></thead><tbody>${processStreams('PFD').map(stream => `<tr><td><strong>${stream.number}</strong><span class="stream-role">${stream.role}</span></td><td>${esc(stream.from)} → ${esc(stream.to)}</td><td><input data-stream-field="label" data-stream-key="${stream.key}" value="${esc(stream.data.label)}" aria-label="Label for ${stream.number}"></td></tr>`).join('')}</tbody></table></div>`;
}
function renderPick() {
  const issues = processIssues(), valid = processValidated();
  return shell('Prepare and validate the process flow', 'Set the correct equipment order and stream connections before entering numerical stream data or starting balances.',
    `<section class="task-card process-builder-card"><div class="task-head"><div class="task-number">02</div><div><h2>Assemble the process train</h2><p>Add separation units and accessories, then use the arrows to put them in the actual direction of material flow.</p></div></div><div class="process-toolbox"><button class="primary small" data-add-kind="unit">+ Separation unit</button><button class="tool-button pump-tool" data-add-kind="pump">+ Pump</button><button class="tool-button heat-tool" data-add-kind="heat_exchanger">+ Heat exchanger</button><span>Maximum six equipment blocks</span></div><div class="process-block-list">${state.processBlocks.map(renderBlockEditor).join('')}</div><div class="field full"><label for="selectionRationale">Why is this arrangement and target unit suitable?</label><textarea id="selectionRationale" data-field="selectionRationale" rows="3" placeholder="Relate the order, feed behaviour, product quality, utilities, safety and operating mode to the project brief.">${esc(state.selectionRationale)}</textarea></div></section>
    <section class="task-card diagram-builder"><div class="task-head"><div class="task-number">03</div><div><h2>Check the BFD and PFD</h2><p>The BFD shows primary separation steps. The PFD adds pumps and heat exchangers in their real process order.</p></div></div><div class="diagram-toolbar"><div class="toggle-group"><button class="diagram-type ${state.diagramView === 'BFD' ? 'active' : ''}" data-flow-view="BFD">Block Flow Diagram</button><button class="diagram-type ${state.diagramView === 'PFD' ? 'active' : ''}" data-flow-view="PFD">Process Flow Diagram</button></div><button class="primary small" id="downloadDiagram">Download SVG</button></div><div class="diagram-canvas">${renderProcessDiagram()}</div><div class="legend-row"><span><i class="legend-unit"></i> Separation unit</span><span><i class="legend-pump"></i> Pump</span><span><i class="legend-heat"></i> Heat exchanger</span><span>★ Target for sizing</span></div></section>
    <section class="task-card"><div class="task-head"><div class="task-number">04</div><div><h2>Label every material stream</h2><p>Stream numbers are generated automatically. Give each connection an engineering name before data entry begins.</p></div></div>${renderStreamLabelTable()}<div class="validation-panel ${valid ? 'valid' : ''}"><button class="primary" id="validateProcess">${valid ? '✓ Process flow validated' : 'Validate process flow'}</button><div>${valid ? '<strong>Flow gate passed.</strong><span>The current order, tags, target unit and stream labels are complete.</span>' : `<strong>${issues.length ? `${issues.length} item${issues.length === 1 ? '' : 's'} to resolve` : 'Ready to validate'}</strong><span>${issues.length ? esc(issues.join(' ')) : 'Run the validation to lock this process definition for data entry.'}</span>`}</div></div><p class="formula-note">Educational boundary: this PFD defines equipment sequence and material streams. Detailed piping, valves, control loops and relief design belong in later design work.</p></section>`);
}
function renderStreamDataTable() {
  ensureStreamData(); const rows = processStreams('PFD').map(stream => `<tr><td><strong>${stream.number}</strong><span class="data-meta">${esc(stream.data.label)}</span></td><td>${esc(stream.from)} → ${esc(stream.to)}</td><td><select data-stream-field="phase" data-stream-key="${stream.key}">${optionList(['Liquid','Vapour','Solid','Slurry','Mixed'], stream.data.phase || 'Liquid')}</select></td><td><input type="number" step="any" min="0" data-stream-field="flow" data-stream-key="${stream.key}" value="${hasNumber(stream.data.flow) ? stream.data.flow : ''}" placeholder="Unknown">${stream.data.flowCalculated ? '<span class="calculated-badge">calculated</span>' : ''}</td><td><input data-stream-field="composition" data-stream-key="${stream.key}" value="${esc(stream.data.composition || '')}" placeholder="0.10, 0.90">${stream.data.compositionCalculated ? '<span class="calculated-badge">pass-through</span>' : ''}</td><td><input type="number" step="any" data-stream-field="temperature" data-stream-key="${stream.key}" value="${esc(stream.data.temperature || '')}" placeholder="°C"></td><td><input type="number" step="any" data-stream-field="pressure" data-stream-key="${stream.key}" value="${esc(stream.data.pressure || '')}" placeholder="kPa"></td></tr>`).join('');
  return `<div class="data-table-wrap wide-table"><table class="data-table stream-data-table"><thead><tr><th>STREAM</th><th>CONNECTION</th><th>PHASE</th><th>FLOW (${esc(state.flowUnit)})</th><th>COMPOSITION</th><th>T</th><th>P</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
function renderEstablish() {
  const operation = operations[state.operation], dof = dofSummary();
  return shell('Enter stream and evidence data', 'The validated process definition now generates the stream table. Enter known specifications and leave balance unknowns blank.',
    `<section class="mechanism-band"><div><span>TARGET SEPARATION</span><strong>${operation.mechanism}</strong></div><div><span>PRIMARY DRIVER</span><strong>${operation.driver}</strong></div><div><span>FIRST DESIGN OUTPUT</span><strong>${operation.size}</strong></div></section><section class="task-card"><div class="task-head"><div class="task-number">05</div><div><h2>Set components, basis and assumptions</h2><p>The first listed component is used as the key component in the transparent two-component balance.</p></div></div><div class="field-grid">${field('components', 'Components in order', state.components, 'Example: solute, water. Separate names with commas.', 'text', 'full')}<div class="field"><label>Composition basis</label><select data-field="compositionBasis"><option ${state.compositionBasis === 'Mass fraction' ? 'selected' : ''}>Mass fraction</option><option ${state.compositionBasis === 'Mole fraction' ? 'selected' : ''}>Mole fraction</option></select></div>${field('basis', 'Calculation basis', state.basis, 'Examples: one hour operation or one batch.')}<div class="field full"><span class="group-label">Assumptions used now</span><div class="choice-list"><label class="check-row"><input type="checkbox" data-assumption="steady" ${state.assumptions.includes('steady') ? 'checked' : ''}><span><strong>Steady state</strong><br>No accumulation within each selected boundary.</span></label><label class="check-row"><input type="checkbox" data-assumption="nonvolatile" ${state.assumptions.includes('nonvolatile') ? 'checked' : ''}><span><strong>Key solute is non-volatile</strong><br>Use only when supported by process chemistry.</span></label><label class="check-row"><input type="checkbox" data-assumption="lossless" ${state.assumptions.includes('lossless') ? 'checked' : ''}><span><strong>Negligible material loss</strong><br>No unaccounted leakage, entrainment or hold-up.</span></label></div></div></div></section><section class="task-card"><div class="task-head"><div class="task-number">06</div><div><h2>Complete the generated stream table</h2><p>Enter feed data and specified outlet compositions. Leave calculated flow rates blank. Accessory outlet composition may remain blank because it passes through.</p></div></div>${renderStreamDataTable()}<div class="dof-panel ${dof.dof === 0 ? 'balanced' : dof.dof > 0 ? 'under' : 'over'}"><div><span>UNKNOWN VARIABLES</span><strong>${dof.unknowns}</strong></div><div><span>INDEPENDENT EQUATIONS</span><strong>${dof.equations}</strong></div><div><span>DEGREE OF FREEDOM</span><strong>${dof.dof}</strong></div><p>${dof.dof === 0 ? '✓ Exactly specified: the balance can be attempted.' : dof.dof > 0 ? `Add ${dof.dof} independent specification${dof.dof === 1 ? '' : 's'}.` : `Remove ${Math.abs(dof.dof)} redundant specification${dof.dof === -1 ? '' : 's'}.`}</p></div></section><section class="task-card"><div class="task-head"><div class="task-number">07</div><div><h2>Build the Design Data Register</h2><p>Find missing properties, correlations, operating limits and costs in journals, validated databases, standards or industrial sources. Record conditions and applicability.</p></div></div><div class="source-explainer"><div><strong>FIND</strong><span>Select a suitable external source.</span></div><div><strong>VERIFY</strong><span>Check system, range, conditions and units.</span></div><div><strong>JUSTIFY</strong><span>Explain why it applies to this design.</span></div></div>${renderDataRegister()}</section>`);
}
function renderCalculate() {
  const dof = dofSummary(), solved = state.networkBalance?.status === 'pass' && state.networkBalance.fingerprint === networkFingerprint(), e = energy(), operation = operations[state.operation], equipment = selectedEquipment(), result = state.sizing?.result;
  const resultRows = solved ? state.networkBalance.results.map(item => `<tr><td>${esc(item.tag)}</td><td>${esc(item.kind === 'unit' ? 'Separation balance' : 'Pass-through accessory')}</td><td>${item.inlet.toFixed(2)}</td><td>${item.main.toFixed(2)}</td><td>${item.side.toFixed(2)}</td><td>${item.closure.toFixed(4)}%</td></tr>`).join('') : '';
  return shell('Calculate balances and preliminary size', 'First solve the material balance generated by the process train; then size the selected target equipment.',
    `<section class="task-card"><div class="task-head"><div class="task-number">08</div><div><h2>Solve the process-stream balances</h2><p>Each binary separation unit applies total and key-component balances. Accessories pass mass and composition through unchanged.</p></div></div><div class="equation"><strong>Unit:</strong> F = P + R &nbsp; and &nbsp; Fz = Px + Ry &nbsp;&nbsp; <strong>Accessory:</strong> Fout = Fin</div><div class="dof-inline"><span>DOF before solution</span><strong>${dof.dof}</strong></div><button class="primary" id="solveNetwork" ${dof.dof === 0 ? '' : 'disabled'}>Solve process balances</button>${state.networkBalance?.status === 'fail' ? `<div class="source-alerts">${state.networkBalance.errors.map(error => `<div class="source-alert">${esc(error)}</div>`).join('')}</div>` : ''}${solved ? `<div class="data-table-wrap"><table class="data-table"><thead><tr><th>EQUIPMENT</th><th>MODEL</th><th>INLET</th><th>MAIN OUT</th><th>SIDE OUT</th><th>CLOSURE</th></tr></thead><tbody>${resultRows}</tbody></table></div>` : '<p class="formula-note">The solve button unlocks only when DOF = 0.</p>'}</section><section class="task-card"><div class="task-head"><div class="task-number">09</div><div><h2>Estimate target-unit thermal duty</h2><p>This sensible-plus-latent screen is most relevant to evaporation and drying; adapt terms for other operations.</p></div></div><div class="field-grid">${field('cp', 'Average heat capacity (kJ kg⁻¹ K⁻¹)', state.cp, '', 'number')}${field('dTfeed', 'Feed temperature change (K)', state.dTfeed, '', 'number')}${field('latent', 'Latent heat (kJ kg⁻¹)', state.latent, 'Record conditions and source.', 'number')}</div><div class="results-grid">${e && solved ? `<div class="result"><span>Sensible duty</span><strong>${e.qSens.toFixed(1)} kW</strong></div><div class="result"><span>Latent duty</span><strong>${e.qLat.toFixed(1)} kW</strong></div><div class="result"><span>Illustrative total</span><strong>${e.total.toFixed(1)} kW</strong></div>` : '<div class="error-item">Solve the material balance first.</div>'}</div></section><section class="task-card sizing-card"><div class="task-head"><div class="task-number">10</div><div><h2>${equipment.label}: preliminary sizing</h2><p>Target ${esc(targetBlock()?.tag || '')}. Design focus: ${equipment.design}.</p></div></div><div class="equation"><strong>Starting relation:</strong> ${operation.formula}</div><div class="field-grid calculation-fields">${sizingInputs()}</div><button class="primary" id="calculateSize" ${solved ? '' : 'disabled'}>Calculate preliminary size</button>${result ? `<div class="results-grid"><div class="result standout"><span>${result.label}</span><strong>${Number(result.value).toFixed(2)} ${result.unit}</strong></div><div class="result"><span>Design allowance</span><strong>${state.sensitivity}%</strong></div><div class="result"><span>Adjusted size</span><strong>${(result.value * state.sensitivity / 100).toFixed(2)} ${result.unit}</strong></div></div>` : ''}<p class="formula-note">This transparent screening relation is not a full mechanical design. Verify equipment-specific geometry, hydraulics, controls and safety limits.</p></section>`);
}
function renderArgue() {
  const econ = state.economics.result, sim = simulationStatus(state.simulation), readiness = [['Scope and target', phaseReady(0)], ['Validated BFD/PFD', phaseReady(1)], ['Stream data and evidence', phaseReady(2)], ['Balances and size', phaseReady(3)], ['Simulation gate', phaseReady(4)], ['Economics and review', phaseReady(5)]];
  return shell('Argue and archive the design', 'Defend the validated process arrangement and target-unit design using evidence, balances, simulation, economics and limitations.', `<section class="task-card diagram-builder"><div class="task-head"><div class="task-number">14</div><div><h2>Final validated process flow</h2><p>This diagram is inherited from the flow-definition stage; return there to change equipment order or stream labels.</p></div></div><div class="diagram-toolbar"><div class="toggle-group"><button class="diagram-type ${state.diagramView === 'BFD' ? 'active' : ''}" data-flow-view="BFD">BFD</button><button class="diagram-type ${state.diagramView === 'PFD' ? 'active' : ''}" data-flow-view="PFD">PFD</button></div><button class="primary small" id="downloadDiagram">Download SVG</button></div><div class="diagram-canvas">${renderProcessDiagram()}</div></section><section class="task-card"><div class="task-head"><div class="task-number">15</div><div><h2>Defensible recommendation</h2><p>Use claim–evidence–limitation and address conditional simulation or economic findings.</p></div></div><div class="evidence-ribbon"><span>${operations[state.operation].name}</span><span>${selectedEquipment().label}</span><span>${sim.label}</span><span>${econ ? `ROI ${econ.roi.toFixed(1)}%` : 'Economics pending'}</span></div><div class="field full"><label for="decision">Final design recommendation</label><textarea id="decision" data-field="decision" rows="6" placeholder="I recommend… The evidence is… The preliminary size is… The simulation shows… The ROI is… The main limitations are…">${esc(state.decision)}</textarea><span class="help">Write at least 40 characters and include numerical evidence.</span></div><div class="info-strip"><div>◆</div><div><strong>Defence prompt:</strong> If feed rate increases by 20%, which calculation changes first and which equipment or economic limit becomes most important?</div></div></section><section class="task-card"><div class="task-head"><div class="task-number">16</div><div><h2>Portfolio readiness</h2><p>The portfolio compiles the complete design trail automatically.</p></div></div><div class="readiness-grid">${readiness.map(([label, ready]) => `<div class="readiness-item ${ready ? 'ready' : ''}"><b>${ready ? '✓' : '○'}</b><span>${label}</span></div>`).join('')}</div><button class="primary" id="openPortfolio">Review complete portfolio</button></section>`);
}

function render() { normalizeProcess(); syncSelection(); const views = [renderScope, renderPick, renderEstablish, renderCalculate, renderTest, renderReview, renderArgue]; document.getElementById('workspace').innerHTML = views[state.phase](); bindWorkspace(); renderNotebook(); renderProgress(); }
function bindWorkspace() {
  document.querySelectorAll('[data-field]').forEach(input => input.oninput = () => {
    ...
    if (['components','compositionBasis','basis','throughput','flowUnit'].includes(input.dataset.field)) { state.networkBalance = null; state.sizing = {}; state.simulation = null; }
  ...
  document.querySelectorAll('[data-add-kind]').forEach(button => button.onclick = () => {
    if (state.processBlocks.length >= 6) { alert('Use no more than six equipment blocks in this preliminary process train.'); return; }
    const kind = button.dataset.addKind, id = uid(), index = state.processBlocks.length;
    state.processBlocks.push(kind === 'unit' ? { id, kind, operation: 'evaporation', equipment: 'falling_film', mode: 'Single effect', configuration: 'Vacuum', tag: `EV-${101 + index}`, target: !targetBlock() } : { id, kind, tag: `${kind === 'pump' ? 'P' : 'E'}-${101 + index}` });
    invalidateProcess(); normalizeProcess(); save(); render();
  });
  document.querySelectorAll('[data-move-block]').forEach(button => button.onclick = () => { const index = state.processBlocks.findIndex(block => block.id === button.dataset.moveBlock), next = index + Number(button.dataset.direction); if (index < 0 || next < 0 || next >= state.processBlocks.length) return; [state.processBlocks[index], state.processBlocks[next]] = [state.processBlocks[next], state.processBlocks[index]]; invalidateProcess(); save(); render(); });
  document.querySelectorAll('[data-remove-block]').forEach(button => button.onclick = () => { const block = state.processBlocks.find(item => item.id === button.dataset.removeBlock); if (!block || !confirm(`Remove ${block.tag} from the process train? Its obsolete stream data will no longer be used.`)) return; state.processBlocks = state.processBlocks.filter(item => item.id !== block.id); invalidateProcess(); normalizeProcess(); save(); render(); });
  document.querySelectorAll('[data-block-field]').forEach(input => input.onchange = () => { const block = state.processBlocks.find(item => item.id === input.dataset.blockId); if (!block) return; block[input.dataset.blockField] = input.value; if (input.dataset.blockField === 'operation') { const op = operations[block.operation]; block.equipment = op.equipment[0].id; block.mode = op.modes[0]; block.configuration = op.configurations[0]; if (!block.tag.trim() || /^[A-Z]{1,3}-\d+$/.test(block.tag)) block.tag = `${op.icon}-${101 + state.processBlocks.indexOf(block)}`; } invalidateProcess(); normalizeProcess(); save(); render(); });
  document.querySelectorAll('[data-target-block]').forEach(input => input.onchange = () => { state.processBlocks.forEach(block => block.target = block.id === input.dataset.targetBlock); invalidateProcess(); normalizeProcess(); save(); render(); });
  document.querySelectorAll('[data-stream-field]').forEach(input => {
    const update = () => { const item = state.streamData[input.dataset.streamKey] ||= {}; item[input.dataset.streamField] = input.value; if (input.dataset.streamField === 'label') state.processValidation = null; else { item[`${input.dataset.streamField}Calculated`] = false; state.networkBalance = null; state.sizing = {}; state.simulation = null; } save(); };
    input.oninput = update; input.onchange = () => { update(); render(); };
  });
  document.querySelectorAll('[data-flow-view]').forEach(button => button.onclick = () => { state.diagramView = button.dataset.flowView; save(); render(); });
  const validateProcess = document.getElementById('validateProcess'); if (validateProcess) validateProcess.onclick = () => { const issues = processIssues(); state.processValidation = issues.length ? null : { status: 'pass', fingerprint: processFingerprint(), validatedAt: new Date().toISOString() }; save(); render(); if (issues.length) alert(issues.join('\n')); };
  const solveNetwork = document.getElementById('solveNetwork'); if (solveNetwork) solveNetwork.onclick = () => { state.networkBalance = solveNetworkBalances(); state.sizing = {}; state.simulation = null; save(); render(); };
  ...
  document.querySelectorAll('[data-add-category]').forEach(button => button.onclick = () => openDataDialog(-1, button.dataset.addCategory));
  document.querySelectorAll('[data-source-url]').forEach(button => button.onclick = () => window.open(button.dataset.sourceUrl, '_blank', 'noopener,noreferrer'));
  document.querySelectorAll('[data-attachment]').forEach(button => button.onclick = () => { const entry = state.dataEntries[Number(button.dataset.attachment)]; if (!entry?.attachmentData) { alert('The embedded file is not available on this device. Use Edit to attach it again.'); return; } const anchor = document.createElement('a'); anchor.href = entry.attachmentData; anchor.download = entry.attachmentName || 'evidence-file'; anchor.click(); });
  ...
  const continueEvidence = document.getElementById('continueEvidence'); if (continueEvidence) continueEvidence.onclick = () => { state.phase = 3; save(); render(); document.getElementById('workspace').focus(); };
  ...
  const next = document.getElementById('nextBtn'); if (next) next.onclick = () => { if (state.phase === phases.length - 1) openPortfolio(); else { state.phase += 1; save(); render(); document.getElementById('workspace').focus(); } };
...
function exportDiagram() {
  ...
  const blob = new Blob([new XMLSerializer().serializeToString(copy)], { type: 'image/svg+xml' }), anchor = document.createElement('a'); anchor.href = URL.createObjectURL(blob); anchor.download = `spectra-${state.diagramView.toLowerCase()}.svg`; anchor.click(); URL.revokeObjectURL(anchor.href);
}

function toggleEvidenceMethod() { const method = document.querySelector('[name="evidenceMethod"]:checked')?.value || 'text'; document.querySelectorAll('[data-evidence-panel]').forEach(panel => { panel.hidden = panel.dataset.evidencePanel !== method; panel.querySelectorAll('input,textarea,select').forEach(input => input.disabled = panel.hidden); }); }
function openDataDialog(index = -1, category = 'other') {
  const sourceDefaults = { brief: 'Project brief', properties: 'Validated database', methods: 'Textbook or handbook', equilibrium: 'Peer-reviewed journal', limits: 'Industrial or vendor datasheet', costs: 'Industrial or vendor datasheet', assumptions: 'Engineering assumption', other: 'Validated database' }, dialog = document.getElementById('dataDialog'), entry = index >= 0 ? state.dataEntries[index] : { inputMethod: 'text', category, name: '', value: '', unit: '', conditions: '', sourceType: sourceDefaults[category] || sourceDefaults.other, status: category === 'assumptions' ? 'Assumed' : 'Provisional', reference: '', applicability: '', url: '' };
  ...
  document.querySelector(`[name="evidenceMethod"][value="${entry.inputMethod || 'text'}"]`).checked = true; document.getElementById('dataCategory').value = entry.category || category || 'other'; document.getElementById('dataUrl').value = entry.url || ''; document.getElementById('dataFile').value = ''; document.getElementById('dataAttachmentNote').textContent = entry.attachmentName ? `Current attachment: ${entry.attachmentName}. Choose another file to replace it.` : 'Files up to 1 MB are stored inside this local project. For larger files, use a shared URL.';
  document.getElementById('dataSourceType').value = entry.sourceType; document.getElementById('dataStatus').value = entry.status || 'Provisional'; document.getElementById('dataFormFeedback').textContent = ''; toggleEvidenceMethod(); dialog.showModal(); setTimeout(() => document.getElementById('dataName').focus(), 0);
}
async function saveDataEntry(event) {
  event.preventDefault(); const index = Number(document.getElementById('dataIndex').value), previous = index >= 0 ? state.dataEntries[index] : {}, inputMethod = document.querySelector('[name="evidenceMethod"]:checked')?.value || 'text', file = document.getElementById('dataFile').files[0], url = document.getElementById('dataUrl').value.trim();
  const entry = { ...previous, inputMethod, category: document.getElementById('dataCategory').value, name: document.getElementById('dataName').value.trim(), value: document.getElementById('dataValue').value.trim(), unit: document.getElementById('dataUnit').value.trim(), conditions: document.getElementById('dataConditions').value.trim(), sourceType: document.getElementById('dataSourceType').value, status: document.getElementById('dataStatus').value, reference: document.getElementById('dataReference').value.trim(), applicability: document.getElementById('dataApplicability').value.trim(), url: inputMethod === 'url' ? url : '' };
  const immediateIssues = [];
  if (inputMethod === 'text' && !entry.value) immediateIssues.push('Paste or type the information, value, equation or method.');
  if (inputMethod === 'url' && !safeHttpUrl(url)) immediateIssues.push('Enter a complete http or https URL.');
  if (inputMethod === 'file' && !file && !previous.attachmentData) immediateIssues.push('Select a file to attach.');
  if (file && file.size > 1024 * 1024) immediateIssues.push('The file is larger than 1 MB. Upload it to Drive or another approved location and use URL / DOI instead.');
  if (immediateIssues.length) { document.getElementById('dataFormFeedback').textContent = immediateIssues.join(' '); return; }
  if (inputMethod === 'url' && !entry.reference) entry.reference = url;
  if (inputMethod === 'file') { if (file) { try { entry.attachmentData = await readFileAsDataURL(file); entry.attachmentName = file.name; entry.attachmentType = file.type; entry.attachmentSize = file.size; } catch (error) { document.getElementById('dataFormFeedback').textContent = error.message; return; } } if (!entry.reference) entry.reference = `Attached file: ${entry.attachmentName}`; }
  else { delete entry.attachmentData; delete entry.attachmentName; delete entry.attachmentType; delete entry.attachmentSize; }
  if (entry.sourceType === 'Engineering assumption') entry.status = 'Assumed';
  if (index >= 0) state.dataEntries[index] = entry; else state.dataEntries.push(entry); state.showDataQuality = true; save(); document.getElementById('dataDialog').close(); render();
}

function legacyRenderNotebook() {
...
function renderNotebook() {
  const target = targetBlock(), result = state.sizing?.result, verified = state.dataEntries.filter(entry => entry.status === 'Verified').length, sim = simulationStatus(state.simulation), econ = state.economics.result, dof = dofSummary();
  document.getElementById('notebookContent').innerHTML = `<section class="notebook-section"><h3>S · SCOPE</h3><div class="note-row"><span>Project</span><strong>${esc(state.projectTitle || '—')}</strong></div><div class="note-row"><span>Throughput</span><strong>${state.throughput} ${state.flowUnit}</strong></div></section><section class="notebook-section"><h3>P · PREPARE FLOW</h3><div class="note-row"><span>Process train</span><strong>${state.processBlocks.map(block => esc(block.tag)).join(' → ')}</strong></div><div class="note-row"><span>Flow validation</span><strong class="text-${processValidated() ? 'pass' : 'fail'}">${processValidated() ? 'Validated' : 'Pending'}</strong></div></section><section class="notebook-section"><h3>E · ENTER DATA</h3><div class="note-row"><span>Streams / DOF</span><strong>${processStreams('PFD').length} / ${dof.dof}</strong></div><div class="note-row"><span>Evidence</span><strong>${state.dataEntries.length} items · ${verified} verified</strong></div></section><section class="notebook-section"><h3>C · CALCULATE</h3><div class="note-row"><span>Network balance</span><strong>${state.networkBalance?.status === 'pass' ? 'Closed' : 'Pending'}</strong></div>${result ? `<div class="note-row"><span>${esc(target?.tag)} · ${result.label}</span><strong>${Number(result.value).toFixed(2)} ${result.unit}</strong></div>` : ''}</section><section class="notebook-section"><h3>T · TEST</h3><div class="note-row"><span>Simulator</span><strong class="text-${sim.cls}">${sim.label}</strong></div></section><section class="notebook-section"><h3>R · REVIEW</h3>${econ ? `<div class="note-row"><span>ROI / payback</span><strong>${econ.roi.toFixed(1)}% / ${Number.isFinite(econ.payback) ? econ.payback.toFixed(2) + ' y' : 'No payback'}</strong></div>` : '<p class="empty-note">Economics not calculated.</p>'}</section><section class="notebook-section"><h3>A · ARGUE</h3>${state.decision ? `<div class="decision-box">${esc(state.decision)}</div>` : '<p class="empty-note">Recommendation pending.</p>'}</section>`;
}
function openPortfolio() {
  ensureStreamData(); const operation = operations[state.operation], equipment = selectedEquipment(), e = energy(), result = state.sizing?.result, econ = state.economics.result, sim = simulationStatus(state.simulation), target = targetBlock();
  const streamRows = processStreams('PFD').map(stream => `<tr><td>${stream.number}</td><td>${esc(stream.data.label)}</td><td>${esc(stream.from)} → ${esc(stream.to)}</td><td>${hasNumber(stream.data.flow) ? Number(stream.data.flow).toFixed(2) : 'Unknown'}</td><td>${esc(stream.data.composition || 'Unknown')}</td><td>${esc(stream.data.temperature || '—')}</td><td>${esc(stream.data.pressure || '—')}</td></tr>`).join('');
  const dataRows = state.dataEntries.length ? state.dataEntries.map(entry => `<tr><td>${esc(entry.name)}</td><td>${esc(entry.value)} ${esc(entry.unit)}</td><td>${esc(entry.conditions)}</td><td>${esc(entry.sourceType)}: ${esc(entry.reference)}</td><td>${esc(entry.status)}</td></tr>`).join('') : '<tr><td colspan="5">No evidence recorded.</td></tr>';
  const balanceRows = state.networkBalance?.results?.map(item => `<tr><td>${esc(item.tag)}</td><td>${item.inlet.toFixed(2)}</td><td>${item.main.toFixed(2)}</td><td>${item.side.toFixed(2)}</td><td>${item.closure.toFixed(4)}%</td></tr>`).join('') || '<tr><td colspan="5">Balance not solved.</td></tr>';
  document.getElementById('portfolioContent').innerHTML = `<div class="portfolio-grid"><section class="portfolio-section"><h3>S · Design scope</h3><p><strong>${esc(state.projectTitle || 'Untitled project')}</strong></p><p>${esc(state.feedDescription)} at ${state.throughput} ${state.flowUnit}. Target: ${esc(state.productTarget)}.</p><p><strong>Constraints:</strong> ${esc(state.constraints || 'Not recorded')}</p></section><section class="portfolio-section"><h3>P · Validated process route</h3><p><strong>Order:</strong> ${state.processBlocks.map(block => esc(block.tag)).join(' → ')}</p><p><strong>Target:</strong> ${esc(target?.tag || '—')} · ${operation.name} · ${equipment.label}</p><p><strong>Rationale:</strong> ${esc(state.selectionRationale || 'Not recorded')}</p></section><section class="portfolio-section full-span"><h3>P · Process flow diagram</h3><div class="diagram-canvas">${renderProcessDiagram('PFD', 'portfolioProcessSvg')}</div></section><section class="portfolio-section full-span"><h3>E · Stream table</h3><div class="data-table-wrap"><table class="data-table"><thead><tr><th>ID</th><th>LABEL</th><th>CONNECTION</th><th>FLOW (${esc(state.flowUnit)})</th><th>COMPOSITION</th><th>T (°C)</th><th>P (kPa)</th></tr></thead><tbody>${streamRows}</tbody></table></div></section><section class="portfolio-section full-span"><h3>E · Design Data Register</h3><div class="data-table-wrap"><table class="data-table"><thead><tr><th>DATA / METHOD</th><th>VALUE</th><th>CONDITIONS</th><th>SOURCE</th><th>STATUS</th></tr></thead><tbody>${dataRows}</tbody></table></div></section><section class="portfolio-section"><h3>C · Network balance</h3><div class="data-table-wrap"><table class="data-table"><thead><tr><th>TAG</th><th>INLET</th><th>MAIN</th><th>SIDE</th><th>CLOSURE</th></tr></thead><tbody>${balanceRows}</tbody></table></div>${e ? `<p>Illustrative target-unit thermal duty: ${e.total.toFixed(1)} kW.</p>` : ''}${result ? `<p>${result.label}: <strong>${Number(result.value).toFixed(2)} ${result.unit}</strong>; adjusted: ${(result.value * state.sensitivity / 100).toFixed(2)} ${result.unit}.</p>` : ''}</section><section class="portfolio-section"><h3>T · Simulation</h3><p><strong>${sim.label}</strong></p>${state.simulation ? `<ul>${state.simulation.checks.map(item => `<li>${item.pass ? 'Pass' : 'Review'}: ${esc(item.label)}</li>`).join('')}</ul>` : '<p>Not run.</p>'}</section><section class="portfolio-section"><h3>R · Economics and review</h3>${econ ? `<p>Installed capital: ${money(econ.capex)}; OPEX: ${money(econ.opex)} per year.</p><p>ROI: <strong>${econ.roi.toFixed(1)}%</strong>; payback: <strong>${Number.isFinite(econ.payback) ? econ.payback.toFixed(2) + ' years' : 'No payback'}</strong>; screening NPV: ${money(econ.npv)}.</p>` : '<p>Economic evaluation incomplete.</p>'}<p><strong>Revision:</strong> ${esc(state.reflection || 'Not recorded')}</p></section><section class="portfolio-section"><h3>A · Recommendation</h3><p>${esc(state.decision || 'Recommendation not recorded.')}</p></section></div>`; document.getElementById('portfolioDialog').showModal();
}
...
document.getElementById('dataForm').onsubmit = saveDataEntry; document.querySelectorAll('[name="evidenceMethod"]').forEach(input => input.onchange = toggleEvidenceMethod); document.getElementById('printBtn').onclick = () => window.print();
...
function registerAgentTools() {
  ...
  register({ name: 'start_design_project', title: 'Start design project', description: 'Open SPECTRA and configure the first separation unit and project title.', inputSchema: { type: 'object', properties: { operation: { type: 'string', enum: Object.keys(operations) }, equipment: { type: 'string' }, mode: { type: 'string' }, projectTitle: { type: 'string', minLength: 3 } }, required: ['operation', 'projectTitle'], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute(input) { if (!operations[input?.operation] || typeof input?.projectTitle !== 'string' || input.projectTitle.trim().length < 3) throw new Error('A valid operation and project title are required.'); const operation = operations[input.operation], first = { id: uid(), kind: 'unit', operation: input.operation, equipment: operation.equipment.some(item => item.id === input.equipment) ? input.equipment : operation.equipment[0].id, mode: operation.modes.includes(input.mode) ? input.mode : operation.modes[0], configuration: operation.configurations[0], tag: `${operation.icon}-101`, target: true }; state.processBlocks = [first]; state.streamData = {}; state.projectTitle = input.projectTitle.trim(); state.phase = 0; invalidateProcess(); normalizeProcess(); save(); showModule(); return { operation: state.operation, equipment: state.equipment, mode: state.mode, projectTitle: state.projectTitle, stage: phases[state.phase].name }; } });
